...
for (i in 1:length(allStates)) {
  apiART <- GET(
    paste("https://developer.nps.gov/api/v1/articles?stateCode=",
          allStates[i],
          "&api_key=",
          key,
          sep = ""))
  art_parsed <- fromJSON(
    rawToChar(apiART$content),
    flatten = TRUE)|>
    as_tibble()
  ...
  totalARTs<-bind_rows(totalARTs,art_parsed)
  }
```
...
for (i in 1:length(allStates)) {
  apiAlerts<- GET(
    paste("https://developer.nps.gov/api/v1/alerts?stateCode=",
          allStates[i],
          "&api_key=",
          key,
          sep = ""))
  parsedAlerts<- fromJSON(rawToChar(apiAlerts$content),flatten = TRUE)|>
    as_tibble()
  parsedAlerts<-parsedAlerts$data|>
    map_if(is.data.frame,list)|>
    map_if(is.null,list)|>
    as_tibble()
  totalAlerts<- bind_rows(totalAlerts,parsedAlerts)
  }

totalAlerts<- totalAlerts|>
  select(-c(relatedRoadEvents))
```
...
for (i in 1:length(allStates)){
  getEventsAPI<-GET(
    paste("https://developer.nps.gov/api/v1/events?stateCode=",
          allStates[i],
          "&api_key=",
          key,
          sep = ""))
...
getNPSpictures<-GET(
    paste("https://developer.nps.gov/api/v1/events?stateCode=",
          ...
          key,
          sep = ""))
parsePICs<-fromJSON(rawToChar(getNPSpictures$content))
parsePICs<-parsePICs$data|>
  map_if(is.data.frame,list)|>
  map_if(is.null,list)|>
  as_tibble()|>
  unnest_longer(types)|>
...
totalNumberOfAlerts<-getNumAlerts(my_wrapper("alerts","CA"))

# numberOfAlertsByCategory<-getc|>
...
test2<-getNumAlertsCat(my_wrapper("alerts","CA"))
